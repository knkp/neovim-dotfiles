...
if [ -f /etc/os-release ]; then
    ...
    case "$ID" in
        ubuntu|debian)
            sudo apt update
            sudo apt install -y neovim python3 python3-pip nodejs npm git ripgrep fzf imagemagick lazygit
            ;;
        arch)
...
if command -v nvim &> /dev/null; then
    ...
    nvim --headless -c 'MasonInstall tailwindcss html tsserver' -c 'qall'
fi
